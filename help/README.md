...
```json
{
  "cmd": ["ggsrun", "exe2", "--onlyresult", "--scriptfile=${file}"],
  "file_regex": "^(..[^:]*):([0-9]+):?([0-9]+)?:? (.*)$",
...
- **CLI Option & Stdin Prompting (v5.3.5)**: Introduced the `--conflict` string flag (accepting `overwrite` or `add`) to the `exe1` and `updateproject` commands. If duplicates are found in an interactive CLI session and no flag is set, `ggsrun` prompts the user for each conflicting file using `pterm.DefaultInteractiveSelect`.
- **MCP Server Schema & Detailed Safe Execution Descriptions (v5.3.5)**: Expanded `exe1` and `updateproject` schemas in the MCP server with the `conflict` property. Updated the `updateproject` tool description to instruct LLM agents to proactively request user approval on the conflict strategy if the user's prompt is ambiguous.
- **MCP scriptid Priority & exe1 Multi-Task Guidance (v5.3.5)**: Instructed LLM agents in the `exe1` description to follow the priority of script ID resolution (explicit script ID > `ggsrun.cfg` configuration > ask user) and dynamically guided agents to use `exe1` for direct script upload-and-execute workflows instead of chain-invoking `updateproject`.
- **TUI Execution target function bugfix (v5.3.5)**: Fixed a bug in `TuiRunExe1` directory execution where the target function name parameter was ignored and fell back to `main()`. Target function name is now parsed dynamically from the TUI input parameters.

...
- **v5.3.10 (June 2026) - Default Safe Cleanup, Separate Sandbox Script, and MCP Hardening**
  ...
  5. **TUI (FD Mode) Execution Enhancements**: Streamlined the execution flow in the TUI filer by removing the cleanup and conflict resolution prompts. Executions now run with sandboxing bypassed (since users can inspect the script before running) and automatically roll back all remote project changes by default, establishing a seamless zero-interaction run cycle.
